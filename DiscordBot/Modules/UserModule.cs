...
namespace DiscordBot.Modules
{
    public class UserModule : ModuleBase
    {
        ...
        */
        ...
        [Alias("temp")]
        public async Task WeatherTemperature(string city)
        {
            JObject weatherResults = await GetWeatherObject(city: city);

            string weather = $"{weatherResults["main"]["temp"].ToString()}";
        ...
        public async Task WeatherWeather(string city)
        {
            try
            {
                JObject weatherResults = await GetWeatherObject(city: city);
                ...
                char[] weatherDescription =
                    weatherResults["weather"][0]["description"].ToString().ToCharArray();
                weatherDescription[0] = char.ToUpper(weatherDescription[0]);

                EmbedBuilder builder = new EmbedBuilder()
                    .WithAuthor(
                        $"{weatherResults["name"]} {weatherResults["sys"]["country"]} | {weatherResults["main"]["temp"]}°C",
                        $"http://openweathermap.org/img/wn/{weatherResults["weather"][0]["icon"]}.png")
                    .WithColor(Color.LightGrey)
                    .WithDescription(
                        $"{new string(weatherDescription)} | Humidity {weatherResults["main"]["humidity"]}%");

                // with a min of {weatherResults["main"]["temp_min"]}°C and max {weatherResults["main"]["temp_min"]}°C, current humidity is {weatherResults["weather"][0]["humidity"]}%

                await ReplyAsync(embed: builder.Build());
            }
            catch (Exception e)
            {
                Console.WriteLine("Failure while generating Weather results.");
                await ReplyAsync("Failed to generate results.");
            }
        }

        public async Task<JObject> GetWeatherObject(string city, string countryCode = "",
            string unit = "metric")
        {
            if (city.Contains("@everyone") || city.Contains("@here") || countryCode.Contains("@everyone") ||
                countryCode.Contains("@here") || unit.Contains("@everyone") || unit.Contains("@here"))
            {
                await Context.Message.DeleteAsync();
                return null;
            }
            ...
            var query = 
                $"https://api.openweathermap.org/data/2.5/weather?q={city}{(countryCode == "" ? string.Empty : $", {countryCode}")}&appid={api}&units={unit}";

            var result = await InternetExtensions.GetHttpContents(query);

            return result == String.Empty ? null : JObject.Parse(result);
        }
