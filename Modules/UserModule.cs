...
namespace DiscordBot.Modules
{
	public class UserModule : ModuleBase
	{
		...
		private async Task Birthday()
		{
			...
			string nextBirthday = "https://docs.google.com/spreadsheets/d/10iGiKcrBl1fjoBNTzdtjEVYEgOfTveRXdI5cybRTnj4/gviz/tq?tqx=out:html&range=C15:C15";
		...
		private async Task Birthday(IUser user)
		{
			...
			string birthdayTable = "https://docs.google.com/spreadsheets/d/10iGiKcrBl1fjoBNTzdtjEVYEgOfTveRXdI5cybRTnj4/gviz/tq?tqx=out:html&gid=318080247&range=B:D";
			...
			foreach (HtmlNode row in doc.DocumentNode.SelectNodes("/html/body/table/tr"))
			{
				...
				if (name.ToLower().Contains(searchName.ToLower()))
				{
					...
					try
					{
						...
						birthdate = DateTime.ParseExact(dateString, wrongFormat, provider).ToString(rightFormat);
					}
					catch (FormatException)
					{
						// Converting the birthdate from the wrong format to the right format WITHOUT year
						birthdate = DateTime.ParseExact(dateString, "M/d", provider).ToString("dd-MMMM");
					}
					break;
				}
			}

			string msg = "**" + searchName + "**'s birthdate: __**" + birthdate + "**__";
